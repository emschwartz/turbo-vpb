...
var create_canvas_qrcode = __webpack_require__(4);

var create_svg_qrcode = __webpack_require__(8);

var kjua = function kjua(options) {
  var settings = Object.assign({}, defaults, options);
  var qr = qrcode(settings.text, settings.ecLevel, settings.minVersion, settings.quiet);
...
try {
  jQuery.fn.kjua = function (options) {
    return this.each(function (idx, el) {
      return el.appendChild(kjua(options));
    });
  };
} catch (e) {}
/* eslint-enable */
...
var qr_gen = __webpack_require__(3);

qr_gen.stringToBytes = qr_gen.stringToBytesFuncs['UTF-8'];

var min_qrcode = function min_qrcode(text, level) {
  var min_ver = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 1;
  min_ver = Math.max(1, min_ver);

  for (var version = min_ver; version <= 40; version += 1) {
    try {
      var _ret = function () {
        var qr = qr_gen(version, level);
        qr.addData(text);
        qr.make();
...
var quiet_qrcode = function quiet_qrcode() {
  var text = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : '';
  var level = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 'L';
  var min_ver = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 1;
  var quiet = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 0;
  var qr = min_qrcode(text, level, min_ver);
  ...
  if (qr) {
    ...
    qr.is_dark = function (row, col) {
      return prev_is_dark(row - quiet, col - quiet);
    };
  }
...
var draw_module_rounded = __webpack_require__(6);

var draw_mode = __webpack_require__(7);

var draw_background = function draw_background(ctx, settings) {
  if (settings.back) {
...
var draw_module_default = function draw_module_default(qr, ctx, settings, width, row, col) {
...
var draw_modules = function draw_modules(qr, ctx, settings) {
  ...
  var draw_module = settings.rounded > 0 && settings.rounded <= 100 ? draw_module_rounded : draw_module_default;
  var mod_count = qr.module_count;
  var mod_size = settings.size / mod_count;
  var offset = 0;
  ...
  for (var row = 0; row < mod_count; row += 1) {
    for (var col = 0; col < mod_count; col += 1) {
...
var draw = function draw(qr, ctx, settings) {
  draw_background(ctx, settings);
  draw_modules(qr, ctx, settings);
  draw_mode(ctx, settings);
};

var create_canvas_qrcode = function create_canvas_qrcode(qr, settings, as_image) {
  var ratio = settings.ratio || dom.dpr;
  var canvas = dom.create_canvas(settings.size, ratio);
...
var get_attr = function get_attr(el, key) {
  return el.getAttribute(key);
};

var set_attrs = function set_attrs(el, obj) {
...
var create_el = function create_el(name, obj) {
  return set_attrs(doc.createElement(name), obj);
};

var create_svg_el = function create_svg_el(name, obj) {
  return set_attrs(doc.createElementNS(SVG_NS, name), obj);
};

var create_canvas = function create_canvas(size, ratio) {
  var canvas = create_el('canvas', {
    width: size * ratio,
    height: size * ratio
  });
...
var canvas_to_img = function canvas_to_img(canvas) {
  var img = create_el('img', {
...
module.exports = {
  ...
  get_attr: get_attr,
  create_el: create_el,
  create_svg_el: create_svg_el,
  create_canvas: create_canvas,
  canvas_to_img: canvas_to_img
};
...
var create_draw_ctx = function create_draw_ctx(ctx) {
  return {
    m: function m(x, y) {
      ctx.moveTo(x, y);
      return this;
...
var draw_dark = function draw_dark(ctx, l, t, r, b, rad, nw, ne, se, sw) {
  if (nw) {
    ctx.m(l + rad, t);
  } else {
    ctx.m(l, t);
...
var draw_light = function draw_light(ctx, l, t, r, b, rad, nw, ne, se, sw) {
  if (nw) {
...
var draw_module_rounded = function draw_module_rounded(qr, ctx, settings, width, row, col) {
  var left = col * width;
  var top = row * width;
  var right = left + width;
  var bottom = top + width;
  var radius = settings.rounded * 0.005 * width;
  var is_dark = qr.is_dark;
  var row_n = row - 1;
  var row_s = row + 1;
  var col_w = col - 1;
  var col_e = col + 1;
  var dark_center = is_dark(row, col);
  var dark_nw = is_dark(row_n, col_w);
  var dark_n = is_dark(row_n, col);
  var dark_ne = is_dark(row_n, col_e);
  var dark_e = is_dark(row, col_e);
  var dark_se = is_dark(row_s, col_e);
  var dark_s = is_dark(row_s, col);
  var dark_sw = is_dark(row_s, col_w);
  var dark_w = is_dark(row, col_w);
  var draw_ctx = create_draw_ctx(ctx);
...
var draw_label = function draw_label(ctx, settings) {
  var size = settings.size;
  var font = 'bold ' + settings.mSize * 0.01 * size + 'px ' + settings.fontname;
  ctx.strokeStyle = settings.back;
  ctx.lineWidth = settings.mSize * 0.01 * size * 0.1;
  ctx.fillStyle = settings.fontcolor;
  ctx.font = font;
  var w = ctx.measureText(settings.label).width;
  var sh = settings.mSize * 0.01;
  var sw = w / size;
  var sl = (1 - sw) * settings.mPosX * 0.01;
  var st = (1 - sh) * settings.mPosY * 0.01;
  var x = sl * size;
  var y = st * size + 0.75 * settings.mSize * 0.01 * size;
  ctx.strokeText(settings.label, x, y);
  ctx.fillText(settings.label, x, y);
};

var draw_image = function draw_image(ctx, settings) {
  var size = settings.size;
...
var draw_mode = function draw_mode(ctx, settings) {
...
var _require = __webpack_require__(5),
    ...
    get_attr = _require.get_attr,
    create_svg_el = _require.create_svg_el;
...
var create_draw_ctx = function create_draw_ctx(ctx) {
  ...
  var rndo = function rndo(x) {
    return Math.round(x * 10) / 10 + ctx.o;
  };

  return {
    m: function m(x, y) {
      ctx.p += "M ".concat(rndo(x), " ").concat(rndo(y), " ");
      return this;
    },
    l: function l(x, y) {
      ctx.p += "L ".concat(rndo(x), " ").concat(rndo(y), " ");
      return this;
    },
    a: function a(x, y, rad) {
      ctx.p += "A ".concat(rnd(rad), " ").concat(rnd(rad), " 0 0 1 ").concat(rndo(x), " ").concat(rndo(y), " ");
      return this;
    }
  };
};
...
var draw_mod = function draw_mod(qr, ctx, settings, width, row, col) {
  var left = col * width;
  var top = row * width;
  var right = left + width;
  var bottom = top + width;
...
var create_path = function create_path(qr, settings) {
  if (!qr) {
    return '';
  }
...
var add_label = function add_label(el, settings) {
  var size = settings.size;
  var font = 'bold ' + settings.mSize * 0.01 * size + 'px ' + settings.fontname;
  ...
  var ratio = settings.ratio || dom.dpr;
  var ctx = dom.create_canvas(size, ratio).getContext('2d');
  ctx.strokeStyle = settings.back;
  ctx.lineWidth = settings.mSize * 0.01 * size * 0.1;
  ctx.fillStyle = settings.fontcolor;
  ctx.font = font;
  ...
  var x = sl * size;
  var y = st * size + 0.75 * settings.mSize * 0.01 * size;
  var text_el = create_svg_el('text', {
    x: x,
    y: y
  });
...
var add_image = function add_image(el, settings) {
  var size = settings.size;
  var w = settings.image.naturalWidth || 1;
  var h = settings.image.naturalHeight || 1;
  var sh = settings.mSize * 0.01;
  ...
  var iw = sw * size;
  var ih = sh * size;
  var img_el = create_svg_el('image', {
...
var create_svg_qrcode = function create_svg_qrcode(qr, settings) {
  var size = settings.size;
  var mode = settings.mode;
  var svg_el = create_svg_el('svg', {
    xmlns: SVG_NS,
    width: size,
    height: size,
    viewBox: "0 0 ".concat(size, " ").concat(size)
  });
  svg_el.style.width = "".concat(size, "px");
  svg_el.style.height = "".concat(size, "px");
