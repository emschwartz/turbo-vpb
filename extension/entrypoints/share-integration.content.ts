...
export default defineContentScript({
  ...
  main() {
    ...
    function parseSettings(): { messageTemplates: MessageTemplate[] } {
      ...
      for (let i = 0; i < templateElements.length; i++) {
        ...
        if (label && message) {
          ...
          console.warn(
            "Invalid message template. Label:",
            label,
            "Message:",
            message,
          );
    ...
    function createShareUrl(messageTemplates: MessageTemplate[]): string {
      return `https://turbovpb.com/share?messageTemplates=${encodeURIComponent(
